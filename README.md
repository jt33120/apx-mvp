...
| `unfit_relevant_share` | float | `0.5` | the share of a sampling run's judged units that, once relevant, declares the *ranking version* **unfit** for this matter — the product says so in words, offers a re-rank with a revised *case theory*, and does **not** offer a line move (FR-23) |
...
| `no-banned-confidence-phrasing` | FR-23 | FR-23 | structural | no_banned_confidence_phrasing | banned phrases + proximity shapes (fr/en/it) in runtime string literals, locale resources and apx/web/src — **LIVE since Story 5.4**, when the sentence it polices shipped. The list had been English-only and green for eleven stories while every user-facing string was French |
...
| `estimator-census-no-bound` | FR-22 | AD-19 | structural | a_census_states_no_bound | `_census_claim_fr` and `_counts_only_claim_fr` in core/domain/statement.py — **the words, which moved there in Story 5.4** — plus `estimate_for_run` in core/domain/sampling.py, the shape. A census is not a tighter bound but a categorically different statement, so its branch carries an exact count and builds no percentage at all, and the bound branch carries no exact count; the crossover is `n == N` exactly and no third register exists near it (Story 5.2, OQ-4 input 2). The check FAILED CLOSED the moment the sentences left sampling.py, which is what a fail-closed check is for |
...
| `audit-catalogue-complete` | FR-24 | AD-43 | structural | audit_catalogue_is_complete | every `_append_audit` call site across apx/** — no verb is ever a literal at the call site (a typo mints an act class nothing counts), every named `ACT_*` exists, and each FR-24 act class either has a real writer or is declared pending with the story that owns it |
| `audit-sequence-not-generated` | FR-53 | AD-43 | structural | audit_sequence_is_not_generated | `Sequence()`, `nextval` and `autoincrement=True` anywhere in apx/** (docstrings exempt, so AD-43 stays quotable), plus the head row in store.py taken `with_for_update` — a burned `nextval` after a worker crash is a permanent gap AD-22 forbids repairing |
| `audit-record-append-only` | FR-24 | AD-22 | structural | audit_record_is_append_only | `delete()`/`update()` built against any of the eight evidential models across apx/**; `audit_chain_head` is exempt and named as such — it is the allocator, not the record |
| `override-reason-one-validator` | FR-25 | AD-37 | structural | override_reason_has_one_validator | every module that names an override act **and** appends to the record, across apx/** — each validates through core/domain/override.py, and no second blank-reason test exists anywhere. "Mandatory" was implemented twice before Story 5.6 (a typed error for pins, a bare ValueError for the truncation override); two copies of one rule drift, and the copy that drifts is the one that stops refusing |
| `override-reason-in-the-record` | FR-25 | AD-22 | structural | override_reason_reaches_the_record | every `_append_audit` call site whose verb is an override, across apx/** — the detail is composed by `override_detail()`, never by hand, so FR-25's verbatim reason cannot be dropped. The truncation override shipped in 5.5 putting its reason on a mutable marker row and not in the record at all |
| `export-tier-never-defaulted` | FR-26 | AD-33 | structural | export_tier_is_never_defaulted | every function across apx/** taking a `tier` parameter — none of them defaults it. The matter export is the one act in the product that can move client content out of the firm on purpose (FR-26 §11); a default decides that on the caller's behalf, and is invisible at the call site where it would be introduced |
| `pending-section-is-not-a-zero` | FR-26 | AD-33 | structural | a_pending_section_is_not_a_zero | the pending sections declared in core/domain/matter_record.py, checked against the act catalogue in **both directions** — a section is pending exactly when its act is uncatalogued — each naming the story that owns it, saying the act does not exist yet, and printing no digit, plus any `len()`/`sum()` over them across apx/**. Zero is a finding about the FIRM; not built is a finding about the BUILD, and a bâtonnier handed the first would draw a conclusion the second does not support |
| `validation-act-sole-acceptor` | FR-45 | AD-33 | structural | only_the_validation_act_accepts | the FR-24 `value_accepted` class and every function across apx/** that appends it — one verb, one writer, and never written without the validation act it must accompany. FR-24 §614: a value the user never touched is recorded as accepted **only** where a validation act occurred over it, so a second writer is not a duplicate but a default, and a default is what FR-45 forbids by name |
| `validation-provenance-never-a-literal` | FR-45 | AD-33 | structural | the_opened_fact_is_never_a_literal | every `opened_at=` call site across apx/** — the opened fact is read per pièce for the acting lawyer, never asserted. A constant is the blanket stamp over a batch that FR-45(c) exists to forbid ("records for each pièce that it was not opened, unless it was"), and it looks perfectly reasonable in review |
| `acceptance-is-never-manufactured` | FR-45 | AD-33 | structural | acceptance_is_never_manufactured | every defined name and French string literal across apx/** — no dwell, scroll-position or screen-visit path to an acceptance (FR-45 forbids each by name), and exactly one home for the sentence the record attributes to the lawyer. A second spelling is a second control whose words are not the ones the record will carry |
| `validation-version-never-defaulted` | FR-45 | AD-33 | structural | the_accepted_version_is_never_defaulted | every call to `validate_pieces` / `batch_split` across apx/** names `version_no`, and no layer that performs or reaches the act declares it with a default. A default resolves whatever ranking version is current AT THE COMMIT, on the record of what a person accepted (AD-23). Scoped to the act: a default is honest on a read and dishonest the moment the answer is written down as an acceptance |
...
| `defer-opens-the-queue` | FR-2 | AD-6 | structural | every_defer_opens_the_queue | every function in the sealed queue package that calls `defer`/`defer_async` — each opens the queue first. `open_async` was called in `manage worker` alone, a **different process**, so the API deferred onto a pool that did not exist: `AppNotOpen` before any network I/O, caught by the upload route's `except Exception` and answered as *« file d'import indisponible »*. Every submission on every real deployment returned 503, and no test could see it — the connector is chosen from `DATABASE_URL` at import time and the suite runs on SQLite, whose in-memory connector has no such guard |
| `rerank-states-its-cost` | FR-22 | AD-6 | structural | every_rerank_enqueue_states_its_cost | every function under `apx/api` that calls `enqueue_ranking` — each first reads what the re-rank will invalidate. A new ranking version moves `ranking_version_no` and `INPUTS_BY_KIND[KIND_SAMPLING_RUN]` is *every* observable, so every open sampling run in the matter is invalidated; `_guard_open_run` is a **write** guard, so the lawyer met the consequence on her next verdict as a 409 — after `abandon_sampling_run` had already audited `verdicts_kept=` the hour she had just lost |
...
| `unfitness-offers-no-line-move` | FR-23 | AD-33 | structural | unfitness_offers_no_line_move | core/domain/statement.py never names the re-line offer; api/app.py ships `unfit_fr`; a client file that can move the line must read it. Leg 3 is vacuous until Story 4.9's line-move surface lands, and says so |
| `inventory-record-fields` | FR-6 | AD-38 | structural | inventory_record_fields_enumerated | Inventory fields in core/domain — seven since Story 5.6 added the identity's third term, `overridden_register_entries`: FR-25 lets a person close a register entry although the document never entered the corpus, and neither of the other two terms can hold it |
