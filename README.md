...
| `audit-record-append-only` | FR-24 | AD-22 | structural | audit_record_is_append_only | `delete()`/`update()` built against any of the seven evidential models across apx/**; `audit_chain_head` is exempt and named as such — it is the allocator, not the record |
