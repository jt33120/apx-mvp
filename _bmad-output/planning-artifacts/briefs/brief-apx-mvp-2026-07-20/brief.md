...
1. **A defensible sentence about what was set aside.** *"200 pieces sampled at random from the 1 400 discarded; none relevant. With 95% confidence, at most 1.5% of the discarded set — about 21 pieces — is relevant."* This is the north star: it converts "recall over precision" from an intention into a number a lawyer can say to a client or to a judge. It is also the direct answer to the only named non-negotiable requirement APX has ever received from a client.

   > **Corrected 21 July 2026.** This sentence previously read *"risk of having missed a relevant document below 1.5%"*. That was false: sampling bounds **prevalence**, not the probability that nothing was missed — at 0 of 200 the bound permits roughly 21 relevant pieces to remain, and being 95% confident that *none* remain would require sampling 1 330 of 1 400. The error originated in the brainstorming session and propagated through every downstream document before an adversarial review caught it. It is recorded rather than quietly fixed, because it is precisely the failure this product exists to prevent: a guess wearing the costume of a proof. The estimator must be hypergeometric, not the binomial rule of three — at a sampling fraction this large the finite-population correction is a real gain, not pedantry.
