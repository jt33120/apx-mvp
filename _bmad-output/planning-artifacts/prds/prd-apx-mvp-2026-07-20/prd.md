...
| §4.4 Triage · §4.6 Worklist | **Contracted**, conditional on §4.9. |
| **FR-19 and FR-23 specifically** | **Blocked on OQ-4.** The sampling ritual, the draw and the *audit record* around them are buildable now; the *numbers* they emit are not, until the estimator question is answered. |
...
This revision absorbed three independent reviews in full rather than dropping findings to keep the increment tidy. The result is **58 functional requirements where there were 36**, for a team of one non-hands-on CTO plus AI agents.
...
- **The APX operator.** There is no admin **cockpit** in this increment — no operator console, no ticketing, no cross-*tenant* view; there is nothing installed to operate. There *is* a minimal per-*tenant* configuration and provisioning surface (FR-50), because a *tenant* that cannot be provisioned and an *RBAC scope* that cannot be granted make the first installation impossible. That surface is the smallest thing that closes the contradiction between FR-30's editability and the absent cockpit; it is not the cockpit.
...
- Correctness is asserted by test against a *corpus* with known plants: every planted match is returned, and no non-match is; plants include accented, hyphenated, elided and OCR-degraded variants.
...
`[NOTE FOR PM]` **This FR and FR-23 are blocked on OQ-4 and cannot be built until it is answered.** The estimator behind both does not exist, no statistician is on the team or budgeted, and OQ-4 is the most statistically load-bearing unresolved item in the increment. **What the increment ships if no answer arrives:** the draw, the verdicts, the *audit record* and the *worklist* mechanics of FR-22 and FR-24 in full, and a sentence reporting **counts only** — *"200 pièces sampled at random from the 1 400 discarded; none relevant"* — with **no bound and no projected figure**. That is a materially weaker product and it is still honest. Shipping a number nobody can defend is neither.
...
A completed sampling run produces the sentence, not a chart. Realizes UJ-2. **Blocked on OQ-4** — see the `[NOTE FOR PM]` at FR-19 for what ships if it is not answered.
...
*(UJ-3's stated premise — Marc knows one pièce in the discarded set is the whole case — had no resolution in the requirements until this FR existed. Retaining that one document required dragging the line past the 400 above it, and recording, as a priced 400-document decision, what was actually a decision about one.)*

---

### 4.10 Reading, validating and handing back the work

**Description.** UJ-1 ends *"She reads the 180 pièces above the line over the weekend."* **Reading is the job**, and until this revision the entire coverage of it was one clause inside FR-11. The product also produced no deliverable: it exported an *audit record* and never the working set, so the associate would re-key 180 references by hand to build her *bordereau* — which is the friction that gets a tool routed around. And "this document was read by a human" — one of the four claims v1 sold and never built — rested on a phrase, *validation act*, that no requirement created.
...
**What has no metric, said plainly.** Justification quality (FR-41) is assessed by review only; *worklist* phrasing (FR-27) is assessed by review only; "self-diagnosing" (§17) and "no technical vocabulary" (§9) have no acceptance criterion. These are real gaps and they are listed here rather than dressed as measurements.
...
- **The CNB guide (17 March 2026)** sets the criteria a *bâtonnier* will actually apply: data located in France or the EU; nationality of the server owner (European, excluding entities subject to extraterritorial laws); location of model hosting; nationality of the model provider; and **systematic verification of AI output**. `[ASSUMPTION: these criteria are reported second-hand from a practitioner reading; obtaining the source PDF is an open action — see OQ-11. They are currently doing load-bearing work.]` Running inside the firm's walls satisfies the first two by construction. **The third and fourth — where the model is hosted, and whose it is — are not satisfied by construction and are not satisfied at all in this increment**, because FR-38 sends *tenant* text to a hosted provider as normal operation (§11). That is the honest position, and OQ-20 is where it is resolved.
  **On the fifth criterion, *systematic verification of AI output*, this document previously claimed satisfaction it did not have.** FR-15 declares *truth status*, FR-24 records decisions and FR-26 exports them — none of which makes verification systematic, and the product's entire proposition is that the lawyer verifies a **sample** rather than 1 400 outputs. **The defensible argument, which is stronger and is now made rather than assumed:** random-sample verification with a stated, sound prevalence bound (FR-22, FR-23, SM-1) is a *better* answer to that criterion than a claim of systematic review that no firm performs and no product can enforce — and it is measurable, whereas "systematic" is not. Combined with per-*pièce* *validation acts* (FR-45) over the *retained set*, the firm can state exactly what was individually verified and what was verified by sample. `[ASSUMPTION: random-sample verification with a stated, sound prevalence bound is a better answer to the CNB's fifth criterion than a systematic-review claim no firm performs. This argument has never been put to a bâtonnier, and the criteria themselves remain second-hand — see A-19 and OQ-11.]`
...
| R-4 | **A cross-*matter* or cross-*tenant* leak.** | A professional-conduct violation, silently, with no error message. The product's entire premise is void. | FR-14 pre-filter **with a scope-mutating adversarial suite**; FR-49 re-stamping on re-scope; FR-1's traversal boundary; FR-29; SM-6 with a zero target; fail-closed (§9); and the security baseline FR-47…FR-51, without which the application-logic controls are irrelevant. | **Medium.** The previous rating — "Low if the tests hold" — was defensible only against the application-logic vector, and only against a suite that did not mutate scopes. Against a stolen laptop, an unencrypted volume, a shared credential or a restored backup, the tests are irrelevant. The rating moves with the controls, not with confidence. |
...
**OQ-4.** **What is the estimator behind the priced statement (FR-19) and the *confidence bound* (FR-23)?** **This is a blocking prerequisite on both FRs, not a nice-to-have** — see the `[NOTE FOR PM]` at FR-19 for what ships without it. The estimand is now settled (§0.2: a prevalence bound, hypergeometric, with its confidence level stated). What remains unresolved is the estimator and its inputs, and it needs a statistician's answer rather than a guess. **It must take all of the following as inputs, each of which a naive estimator gets wrong:**
...
**OQ-6.** **What are the performance targets at 100 000 documents?** Import wall-clock, exhaustive-search latency, time to first ranking. None are stated in the sources; SM-C4 depends on the answer.
...
**OQ-9.** **How are SM-C2 and SM-C3 observed with no telemetry?** They are the counter-metrics that protect the audit surface from becoming noise, and they are precisely the ones a firm would have to volunteer. Currently observable only in evaluation sessions and in what a *tenant* chooses to push.
...
**OQ-13.** **On-premise update delivery.** Signed, offline-installable, reversible migrations against a live 100 000-document index, at a site APX cannot see. Genuinely unsolved (§16). Must be answered before the second installation, not the second increment.

**OQ-14.** **Scale beyond the design target.** Is 100 000 → 1 000 000 documents just more compute? Unanswered in every upstream document.
...
**OQ-19.** **The *audit record* as a sword.** FR-19 requires the priced statement shown at the moment of a line move to be recorded, FR-21 forbids deleting it, and FR-26 exports it — so the firm manufactures and retains forever a dated document in which it was told an estimated prevalence and proceeded. Discoverability, standing under *secret professionnel*, protected-work-product status, and whether a firm would rationally want a retention limit are all unanalysed. **This needs a practitioner's view before build, not after**, and a client's own counsel will raise it before signing.
