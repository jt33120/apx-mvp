...
**Consequences (testable):**
- For every *matter* and for every *tenant*, **counted in *pièces* after container expansion**: `submitted = in corpus + open failure register entries + declared exclusions`, at all times, **with no fourth bucket and no unnamed remainder**. Each of the three terms is separately countable, separately inspectable, and displayed as its own line; nothing may be in two terms or in none. The unit and the freezing point are defined in FR-57; the exclusion term is the declared class below. Asserted by an invariant test that runs after every *import job* and after every retry, at the *design target*.

  > **Corrected 21 July 2026.** This consequence previously read `submitted = in corpus + open failure register entries` … *"with no third bucket"*, which the declared exclusion class immediately below then violates — the invariant test would have failed on the first `.DS_Store`. The intent was never two buckets; it was **no unaccounted-for remainder**. Three named, countable terms satisfy that intent, and a fourth would not. Found during requirements extraction for the epic breakdown.
...
- Changing a configuration value that affects retrieval, ranking or the *confidence bound* is recorded in the *audit record* and marks derived artefacts stale (FR-58). Changing an *RBAC scope* takes effect at the next query with nothing to propagate (FR-49 as amended, AD-13) — a moved wall needs no staleness marking because no stamp exists to go stale.
...
- Encryption is a property of the deployment the operator controls, not of a hosting provider's volume service, so that it holds identically on a firm's own machine (`addendum.md` §1). *(Amended 21 July 2026 — see below.)*
- **Two searchable surfaces are named exceptions to application-layer encryption: the vector column and the deterministic text index.** They are protected by volume- or cluster-level encryption instead. Everything else — original *pièces*, extracted full text, OCR images, the *audit record*, the *failure register*, configuration and staged exports — is encrypted by the application's storage adapters.
- Asserted by test: an inspection of the raw stores for seeded unique tokens finds none in plaintext, **excluding the two named surfaces**, which are asserted differently — a deployment whose data volume is not encrypted fails to start, and the check is part of the same startup gate.
- A deployment started with encryption disabled or without a key fails to start. There is no permissive default and no warning-and-continue. This now covers both layers: application keys **and** volume encryption.

  > **Amendment, 21 July 2026 — decided during architecture (spine open question 1).** As originally written this consequence was unsatisfiable alongside FR-12 and FR-13: an HNSW index cannot be built over an encrypted `halfvec` column, and a text index cannot be built over ciphertext. Every product that offers search meets this wall. **Decision: those two surfaces are encrypted at the volume or cluster layer, on a machine the firm owns — which is not a third party's volume service and therefore satisfies this requirement's intent** (a stolen disk or a restored backup yields nothing) while leaving both indexes buildable. The exception is stated here rather than left implicit, because a security questionnaire will ask and the honest answer must already be written down.
...
- **A re-scope takes effect immediately, with nothing to propagate.** *(Amended 21 July 2026 — see the note below.)* Changing the *RBAC scope* of a *matter* changes what every subsequent query returns, at the next query, with no propagation step, no migration, and no window in which the material is half-stamped. The re-scope is recorded in the *audit record* as a single operation with its before and after scope. Material is never left with a stale stamp that the pre-filter then enforces correctly and permanently against the wrong wall. This is asserted by the mutating adversarial suite of FR-14.

  > **Amendment, 21 July 2026 — the guarantee is unchanged and strictly stronger; the mechanism is gone.** This requirement previously specified re-stamping: *"propagates to every chunk of that matter, or invalidates the affected material until it is re-stamped."* Architecture decision **AD-13** removes re-stamping from the system — *RBAC scope* is resolved at query time from a single authoritative source and is never denormalised onto indexed rows, so a re-scope is effective at the next query rather than eventually, and the long-running rewrite that could fail halfway on an unreachable machine does not exist to fail. **The behavioural guarantee this FR asks for is therefore met more strongly than as written.** FR-14's reference to "FR-49 for the re-stamp mechanism" should be read as the grant-and-revocation mechanism, and its adversarial suite must assert the query-time path rather than a propagation that no longer occurs.
...
| R-4 | **A cross-*matter* or cross-*tenant* leak.** | A professional-conduct violation, silently, with no error message. The product's entire premise is void. | FR-14 pre-filter **with a scope-mutating adversarial suite**; FR-49 query-time scope resolution, so no stale stamp can exist (AD-13); **FR-1's scope ceiling and traversal boundary — the guard on the *ingestion* gesture, which is the widest attack surface in the product (§4.1) and not an edge case at the margin of a convenience feature**; FR-29; SM-6 with a zero target; fail-closed (§9); and the security baseline FR-47…FR-51, without which the application-logic controls are irrelevant. | **Medium.** The previous rating — "Low if the tests hold" — was defensible only against the application-logic vector, and only against a suite that did not mutate scopes. Against a stolen laptop, an unencrypted volume, a shared credential or a restored backup, the tests are irrelevant. The rating moves with the controls, not with confidence. |
