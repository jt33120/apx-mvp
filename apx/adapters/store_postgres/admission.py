...
(whole-piece) chunks through the single ``write_chunk`` seam. Both the async worker
...
def admit(
    ...
    cfg = str(store.get_config(tenant, "chunking_config_version"))
    ...
    reshaped, embedded = embed_result(
        ...
        embedder, chunking_config_version=cfg,
    ...
    # the single write_chunk seam — one embedded (whole-piece) chunk per newly-admitted piece (2.8)
    chunks = ChunkStore(store._sf, schema_version=SCHEMA_VERSION, chunking_config_version=cfg)
    for ec in embedded:
        chunks.write_chunk(
