...
def test_reimport_leaves_corpus_unchanged_prior_piece_unmodified_and_reports_already_present(
    ...
    out1 = store.save(IngestionResult(pieces=[p]), scope=SCOPE)
    ...
    out2 = store.save(IngestionResult(pieces=[later]), scope=SCOPE)
...
def test_same_content_two_paths_is_one_piece_with_both_provenance_paths(store: SqlStore) -> None:
    ...
    assert a.id == b.id
    store.save(IngestionResult(pieces=[a, b]), scope=SCOPE)  # both in one job (a folder walk)
...
def test_same_file_in_two_matters_is_two_pieces(store: SqlStore) -> None:
    store.save(IngestionResult(pieces=[_piece("piece", prov="x.txt", matter="m-a")]), scope=SCOPE)
    store.save(IngestionResult(pieces=[_piece("piece", prov="x.txt", matter="m-b")]), scope=SCOPE)
...
def test_an_induced_write_conflict_leaves_exactly_one_copy_and_does_not_fail(
    ...
    store.save(r, scope=SCOPE)  # the first worker commits the one copy
    ...
    out = store.save(r, scope=SCOPE)  # must NOT raise — the genuine duplicate is absorbed
...
def test_a_genuine_integrity_failure_fails_loudly_never_a_silent_drop(store: SqlStore) -> None:
    ...
    with pytest.raises(IntegrityError):
        store.save(IngestionResult(pieces=[bad]), scope=SCOPE)
    with store._sf() as s:
