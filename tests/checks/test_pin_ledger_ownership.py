"""The pin append-only / one-owner gate (Story 4.11, FR-43/AD-37/AD-7): a PinEntry construction
outside the store adapter, or any UPDATE/DELETE of the table, fails the build. Passes the real tree;
fires on fixtures; fails closed."""

from __future__ import annotations

from pathlib import Path

from apx.checks.pin_ledger_ownership import pin_ledger_is_append_only
...
def test_passes_the_real_tree() -> None:
    assert pin_ledger_is_append_only().ok


def test_fires_on_a_construction_outside_the_store_adapter(tmp_path: Path) -> None:
    src = "def leak():\n    return PinEntry(id='x', tenant='t', matter='m')\n"
    r = pin_ledger_is_append_only([_mod(tmp_path, "leak", src)])
...
def test_fires_on_a_core_update_of_the_table(tmp_path: Path) -> None:
    src = ("from sqlalchemy import update\n"
           "def bad(s):\n    s.execute(update(PinEntry).values(action='removed'))\n")
    r = pin_ledger_is_append_only([_mod(tmp_path, "upd", src)])
    assert not r.ok and "UPDATE/DELETE" in r.detail


def test_fires_on_a_raw_delete_sql(tmp_path: Path) -> None:
    src = 'def bad(conn):\n    conn.execute("DELETE FROM pin_entry")\n'
    r = pin_ledger_is_append_only([_mod(tmp_path, "raw", src)])
    assert not r.ok and "UPDATE/DELETE" in r.detail
...
def test_fires_on_attribute_mutation_of_a_loaded_instance(tmp_path: Path) -> None:
    src = ("from sqlalchemy import select\n"
           "def bad(s):\n"
           "    row = s.scalar(select(PinEntry))\n"
           "    row.action = 'removed'\n")
    r = pin_ledger_is_append_only([_mod(tmp_path, "instmut", src)])
