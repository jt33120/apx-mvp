...
# The recall draw over HTTP moved to the sampling run in Story 5.1 (decision A1): the population
# is the Epic-4 DERIVED discarded set, which needs a ranked order and a placed line — neither of
# which this ingest-focused module builds. The end-to-end HTTP path is covered in
# tests/api/test_sampling_api.py, and the store path in tests/adapters/test_sampling.py.
